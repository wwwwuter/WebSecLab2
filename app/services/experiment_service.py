...
class ExperimentService:
    ...
    def create_experiment(user_id, vulnerability_id, experiment_name, target, dvwa_level='low'):
        ...
        ExperimentService._add_log(
            ...
            f'创建实验: {experiment.experiment_name}'
        )
    ...
    def start_experiment(experiment_id, user_id):
        ...
        if experiment.status != Experiment.STATUS_CREATED:
            return None, f'当前状态 "{experiment.status_label}" 不允许启动'

        experiment.status = Experiment.STATUS_RUNNING
        db.session.commit()

        ExperimentService._add_log(experiment.id, 'start', '实验已启动')
        return experiment, None
    ...
    def complete_experiment(experiment_id, user_id, result_text, status='success'):
        ...
        experiment.result = result_text.strip() if result_text else ''
        experiment.completed_time = datetime.now()
        db.session.commit()

        status_label = '成功' if status == 'success' else '失败'
        ExperimentService._add_log(
            experiment.id, 'complete',
            f'实验{status_label}: {result_text[:100]}'
        )
        return experiment, None
    ...
    def close_experiment(experiment_id, user_id):
        ...
        if experiment.status == Experiment.STATUS_CLOSED:
            return None, '实验已经关闭'

        experiment.status = Experiment.STATUS_CLOSED
        if not experiment.completed_time:
            experiment.completed_time = datetime.now()
        db.session.commit()

        ExperimentService._add_log(experiment.id, 'close', '实验已关闭')
        return experiment, None
    ...
    def get_admin_statistics():
        """获取全局实验统计 (管理员), 含 DVWA 等级分布/成功率/近7天新增 (供 KRI 卡与图表)"""
